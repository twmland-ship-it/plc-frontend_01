...
export default defineComponent({
  ...
  setup() {
    ...
    const tableData = computed(() =>
      state.btu.metersTableData.map((el) => {
        return {
          name: el.name,
          action: (
            <ActionSpan>
    ...
    const search = (e) => {
      dispatch("btu/filterMeterTable", e.target.value);
    };
    ...
    const saveSetting = async () => {
      try {
        let title;
        if (settings.id) {
          await dispatch("btu/editMeter", settings);
          title = "修改成功";
        } else {
          await dispatch("btu/addMeter", settings);
          title = "新增成功";
        }
        modal.value = false;
    ...
    const deleteItem = (id) => {
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            await dispatch("btu/deleteMeter", id);
            Modal.success({
              content: "刪除成功",
            });
          } catch (err) {
