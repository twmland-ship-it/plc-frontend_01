  ...
  setup() {
    ...
    onMounted(async () => {
      const res = await Promise.all([
        dispatch("database/getHistorySchedule"),
    ...
    const tableData = computed(() =>
      state.database.historyScheduleTableData.map((el) => {
        return {
          name: el.name,
    ...
    const search = (e) => {
      dispatch("database/filterHistorySchedule", e.target.value);
    };
    ...
    const openAddModal = () => {
      ...
      Object.assign(searchFormState, searchObj);
      modal.value = true;
    };
    const openEditModal = ({
      id,
    ...
    const submitForm = async () => {
      try {
        ...
        if (
          searchFormState.tags.length === 0 &&
          searchFormState.groups.length === 0
        ) {
          failedArr.push("查詢設定");
        }
        ...
        if (params.id) {
          title = "編輯成功";
          await dispatch("database/editHistorySchedule", params);
        } else {
          title = "新增成功";
          await dispatch("database/addHistorySchedule", params);
        }
        submitingForm.value = false;
        modal.value = false;
    ...
    const deleteWork = async (id) => {
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            await dispatch("database/deleteHistorySchedule", id);
            notification.success({
              message: "刪除成功",
            });
          } catch (err) {
    ...
    provide("searchForm", searchFormRef);
    ...
    const updateSearchForm = (data) => {
      Object.assign(searchFormState, data);
    };
